...
def mark_cartao_pago(mes, pago):
    ws = get_ws("Cartoes")
    headers = ws.row_values(1)
    if 'Pago' not in headers:
        ws.update_cell(1, len(headers) + 1, 'Pago')
        headers.append('Pago')
    col = headers.index('Pago') + 1
    meses_col = ws.col_values(1)
    if mes in meses_col:
        ws.update_cell(meses_col.index(mes) + 1, col, pago)
