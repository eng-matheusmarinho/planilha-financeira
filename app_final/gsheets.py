...
# Colunas de pago por mês (TRUE/FALSE)
PAGO_COLS  = MESES
...
def get_valor_mes(row, mes):
    """Retorna o valor do gasto fixo para o mês — usa Valor_{mes} se existir, senão Valor base."""
    col = f'Valor_{mes}'
    v = row.get(col, None)
...
def update_gasto_fixo(df_row_idx, nome, cat, pgto, val):
    ws = get_ws("Gastos_Fixos")
