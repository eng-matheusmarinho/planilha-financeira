...
# ════════════════════════════════════════════════════════════════════════════
    ...
    if df_fixos.empty:
        ...
        for i, row in df_fixos.iterrows():
            ...
            # Valor do mês editável inline
            novo_val_mes = c3.number_input(
                "", value=val_mes, min_value=0.0, step=10.0,
                key=f"valmes_{i}", label_visibility='collapsed'
            )
            if novo_val_mes != val_mes:
                gs.update_valor_mes(i, mes_sel, novo_val_mes)
                st.session_state.df_fixos.at[i, f'Valor_{mes_sel}'] = novo_val_mes
            ...
            with c5:
                ...
                if b1.button(icon, key=f"pago_{i}", use_container_width=True, help="Marcar/desmarcar pago"):
                    ...
                    st.rerun()
                if b2.button("✏️", key=f"edit_{i}", use_container_width=True, help="Editar nome/categoria"):
