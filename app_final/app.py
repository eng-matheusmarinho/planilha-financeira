        ...
        for i, row in df_fixos.iterrows():
            ...
            if st.session_state.get(f'editing_{i}'):
                with st.form(f"edit_form_{i}"):
                    st.caption(f"Editando: **{row.get('Nome','')}**")
                    ...
                    st.markdown(f"**Valores por mês** — edite apenas os meses necessários")
                    cols_m = st.columns(4)
                    novos_vals = {}
                    for j, mes in enumerate(MESES):
                        val_atual = gs.get_valor_mes(row, mes)
                        novos_vals[mes] = cols_m[j % 4].number_input(
                            mes[:3], value=val_atual, min_value=0.0, step=10.0,
                            key=f"vmes_{i}_{mes}"
                        )
                    ...
                    if sc1.form_submit_button("💾 Salvar", use_container_width=True, type="primary"):
                        ...
                        for mes, val in novos_vals.items():
                            gs.update_valor_mes(i, mes, val)
