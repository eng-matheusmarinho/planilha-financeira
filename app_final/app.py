...
with st.sidebar:
    ...
    st.divider()
    if st.button("🔄 Atualizar dados", use_container_width=True, help="Busca dados mais recentes do Google Sheets"):
        refresh()
        ...
        for i, row in df_fixos.iterrows():
            ...
            if st.session_state.get(f'editing_{i}'):
                with st.form(f"edit_form_{i}"):
                    ...
                    novo_val_base = ec4.number_input("Valor base (todos os meses)", value=float(row.get('Valor',0)), min_value=0.0, step=10.0)
                    novo_val_mes = st.number_input(
                        f"Valor específico para {mes_sel} (deixe 0 para usar o valor base)",
                        value=float(row.get(f'Valor_{mes_sel}', 0) or 0),
                        min_value=0.0, step=10.0
                    )
                    sc1, sc2 = st.columns(2)
                    if sc1.form_submit_button("💾 Salvar", use_container_width=True, type="primary"):
                        gs.update_gasto_fixo(i, novo_nome, nova_cat, novo_pgto, novo_val_base)
                        if novo_val_mes > 0:
                            gs.update_valor_mes(i, mes_sel, novo_val_mes)
