...
# ════════════════════════════════════════════════════════════════════════════
    ...
    if df_fixos.empty:
        ...
        # Cabeçalho
        h1,h2,h3,h4,h5 = st.columns([3,2,1.5,1.5,2.5])
        for h,t in zip([h1,h2,h3,h4,h5],['Nome','Categoria','Valor','Status','Ações']):
        ...
        for i, row in df_fixos.iterrows():
            ...
            c1, c2, c3, c4, c5 = st.columns([3, 2, 1.5, 1.5, 2.5])
            ...
            with c5:
                ...
                if b1.button(label, key=f"pago_{i}", use_container_width=True):
                    with st.spinner(""):
                        gs.mark_pago(i, mes_sel, not pago)
                    refresh()
                if b2.button("✏️", key=f"edit_{i}", use_container_width=True):
                    st.session_state[f'editing_{i}'] = True
                if b3.button("🗑️", key=f"del_fixo_{i}", use_container_width=True):
                    with st.spinner(""):
...
# ════════════════════════════════════════════════════════════════════════════
    ...
    # Carrega status pago por cartão
    ...
    with st.form("cartoes_form"):
        ...
        new_pago = {}
        new_pago['Pago_Inter_Amanda']  = cp1.checkbox("✅ Inter paga",  value=pago_por_cartao['Pago_Inter_Amanda'])
        new_pago['Pago_Nubank_Amanda'] = cp2.checkbox("✅ Nubank paga", value=pago_por_cartao['Pago_Nubank_Amanda'])
        new_pago['Pago_BRB_Amanda']    = cp3.checkbox("✅ BRB paga",    value=pago_por_cartao['Pago_BRB_Amanda'])
        ...
        new_pago['Pago_Inter_Matheus']  = cpm1.checkbox("✅ Inter paga",  value=pago_por_cartao['Pago_Inter_Matheus'])
        new_pago['Pago_Nubank_Matheus'] = cpm2.checkbox("✅ Nubank paga", value=pago_por_cartao['Pago_Nubank_Matheus'])
        new_pago['Pago_Outros']         = cpm3.checkbox("✅ Outros pagos", value=pago_por_cartao['Pago_Outros'])

        novo_total = sum(new_vals.values())
        pago_total = sum(new_vals[c] for c, k in zip(CARTAO_COLS, CARTAO_PAGO_KEYS) if new_pago[k])
        pend_total = novo_total - pago_total
        mc1, mc2, mc3 = st.columns(3)
        mc1.metric("Total faturas", fmt(novo_total))
        ...
        if st.form_submit_button("💾 Salvar faturas", use_container_width=True, type="primary"):
            with st.spinner("Salvando..."):
                ...
                for pago_key, pago_val in new_pago.items():
