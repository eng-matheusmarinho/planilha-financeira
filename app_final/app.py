...
def refresh():
    st.cache_data.clear()
    # Limpa dados do session_state para forçar releitura
    for k in ['df_fixos','df_var','df_cart','df_ent']:
        st.session_state.pop(k, None)
    st.rerun()

# Carrega dados apenas uma vez por sessão — sem reler o Sheets a cada interação
...
# Botão de atualizar dados no sidebar
with st.sidebar:
    st.divider()
    if st.button("🔄 Atualizar dados", use_container_width=True):
        refresh()
    st.caption("Dados carregados uma vez por sessão")
